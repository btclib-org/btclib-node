...
_URL = "https://pypi.org/pypi/btclib-node/2026.9.1/json"
...
def test_an_empty_tag_is_nothing_to_wait_for(
    ...
    assert script.main(["btclib-node", ""]) == 0
...
def test_the_version_asked_for_is_the_one_the_tag_names(
    ...
    assert script.main(["btclib-node", "v2026.9.1"]) == 0
...
def test_a_version_not_served_yet_is_waited_for(
    ...
    assert script.main(["btclib-node", "2026.9.1", "--interval", "15"]) == 0
...
def test_a_version_that_never_arrives_is_the_deadline_speaking(
    ...
    assert script.main(["btclib-node", "2026.9.1", "--timeout", "300"]) == 1
...
def test_the_last_wait_is_cut_to_what_is_left_of_the_deadline(
    ...
    arguments = ["btclib-node", "2026.9.1", "--timeout", "20", "--interval", "15"]
...
def test_a_request_outlasting_the_deadline_leaves_no_negative_sleep(
    ...
    assert script.main(["btclib-node", "2026.9.1", "--timeout", "30"]) == 1
...
def test_the_defaults_are_the_budget_a_release_actually_gets(
    ...
    the deadline state one of their own, and the cases that do not are
    answered well inside any deadline they could have been given, one of
    them at the empty-tag guard before the wait is entered at all. So
    the shipped number could be changed without a red run. This is the
    run the workflow makes.
    """
    index = _index(script, monkeypatch, [])

    assert script.main(["btclib-node", "2026.9.1"]) == 1
...
def test_a_2xx_that_is_not_ok_is_not_the_index_serving_it(
    ...
    a `served` that returns `True` for whatever answers. This is the
    case that kills that one.
    """
    monkeypatch.setattr(
        script,
        "urlopen",
