...
_URL = "https://btclib-node.readthedocs.io/en/v2026.9.1/"
_BUILDS = "https://app.readthedocs.org/projects/btclib-node/builds/"
_AGENT = "btclib-node-readthedocs-wait/1.0"
...
def test_a_build_that_has_not_finished_is_waited_for(
    ...
    assert script.main(["btclib-node", "v2026.9.1", "--interval", "30"]) == 0
...
def test_a_build_that_never_arrives_is_the_deadline_speaking(
    ...
    assert script.main(["btclib-node", "v2026.9.1", "--timeout", "60"]) == 1
...
def test_a_deadline_already_spent_asks_nothing_and_still_annotates(
    ...
    assert script.main(["btclib-node", "v2026.9.1", "--timeout", "0"]) == 1
...
def test_the_last_wait_is_cut_to_what_is_left_of_the_deadline(
    ...
    arguments = ["btclib-node", "v2026.9.1", "--timeout", "40", "--interval", "30"]
...
def test_a_request_outlasting_the_deadline_leaves_no_negative_sleep(
    ...
    assert script.main(["btclib-node", "v2026.9.1", "--timeout", "30"]) == 1
...
def test_the_request_bound_is_cut_to_what_is_left_of_the_deadline(
    ...
    arguments = ["btclib-node", "v2026.9.1", "--timeout", "34", "--interval", "30"]
...
def test_the_defaults_are_the_budget_a_release_actually_gets(
    ...
    assert script.main(["btclib-node", "v2026.9.1"]) == 1
...
def test_the_site_answering_the_page_is_it_serving_the_tag(
    ...
    agent = script.USER_AGENT.format(project="btclib-node")
    assert agent == _AGENT
