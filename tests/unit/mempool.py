...
def test_a_full_mempool_takes_nothing_and_asks_for_nothing():
...
def test_the_same_transaction_twice_is_counted_once():
    mempool = Mempool(Logger(debug=True))
    tx = generate_random_transaction()
    mempool.add_tx(tx)
    mempool.add_tx(tx)
...
def test_removing_what_was_never_there_changes_nothing():
    mempool = Mempool(Logger(debug=True))
...
def test_a_transaction_is_found_by_either_of_its_identifiers():
    mempool = Mempool(Logger(debug=True))
