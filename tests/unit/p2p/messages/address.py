...
def test_empty_addr():
    ...
    assert msg == Addr.deserialize(Message.parse(msg_bytes).payload)
...
def test_empty_addrv2():
    msg = AddrV2([])
    msg_bytes = msg.to_message(MAGIC).serialize()
    assert msg == AddrV2.deserialize(Message.parse(msg_bytes).payload)
...
def test_valid_getaddr():
    msg = Getaddr()
    msg_bytes = msg.to_message(MAGIC).serialize()
    assert msg == Getaddr.deserialize(Message.parse(msg_bytes).payload)
