...
def request(headers=b"", body=BODY):
...
def test_a_well_formed_request_is_dispatched():
    outcome, messages, _ = drive([with_length()])
    assert outcome == "returned"
    assert messages == [([json.loads(BODY)], 0)]


def test_a_batch_is_dispatched_as_it_arrived():
...
def test_a_body_split_across_reads_is_reassembled():
    whole = with_length()
    _, messages, _ = drive([whole[:-10], whole[-10:]])
    assert messages == [([json.loads(BODY)], 0)]


def test_a_request_with_no_body_is_refused():
...
def test_a_content_length_past_the_cap_is_refused():
...
def test_an_unterminated_header_section_is_refused():
...
def test_a_client_that_goes_away_mid_request_is_refused():
...
def test_a_body_shorter_than_its_length_is_waited_for():
    whole = with_length()
...
def test_the_response_is_crlf_framed_and_the_socket_closed():
    async def main():
        ours, theirs = socket.socketpair()
        ours.setblocking(False)
        theirs.setblocking(False)
        loop = asyncio.get_running_loop()
        conn = Connection(loop, ours, SimpleNamespace(messages=[]), 0)
...
def test_a_response_of_several_stays_a_list():
    async def main():
        ours, theirs = socket.socketpair()
        ours.setblocking(False)
        theirs.setblocking(False)
...
def test_the_encoder_defers_to_json_for_what_is_not_bytes():
...
def test_close_cancels_the_task_it_was_given():
    async def main():
        ours, theirs = socket.socketpair()
        ours.setblocking(False)
        loop = asyncio.get_running_loop()
...
def test_repr_names_the_peer_and_says_so_when_there_is_none():
    ...
    conn = Connection(None, client, SimpleNamespace(messages=[]), 0)
...
def test_close_without_a_task_closes_the_socket_anyway():
    ours, theirs = socket.socketpair()
    conn = Connection(None, ours, SimpleNamespace(messages=[]), 0)
...
def test_send_and_wait_gives_up_rather_than_blocking_forever():
