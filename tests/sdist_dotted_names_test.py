...
this tree's floor -- the reason `tests/interpreters_test.py` gives for
...
# resolved once, for the reason a bare "git" in a subprocess list is a
...
def _source_exclude() -> tuple[str, ...]:
    ...
    entries = _PYPROJECT["tool"]["uv"]["build-backend"]["source-exclude"]
    assert entries, "no source-exclude array in pyproject.toml"
