...
_NOTICE = """# Copyright (c) The btclib developers
# Distributed under the MIT software license, see the accompanying
# LICENSE file or https://opensource.org/license/mit for the full text.
#
# The seeds below are Bitcoin Core's, and Core's licence follows in full,
# scripts/seeds/BITCOIN_CORE_COPYING verbatim:
#
"""
...
def generate(indir: Path) -> str:
    ...
    # Core's header points at the COPYING at its tree's root; the wheel
    # carries no copy of this one, so its text goes into the module itself
    copying = (indir / "BITCOIN_CORE_COPYING").read_text("utf-8").splitlines()
    notice = "".join(f"# {line}\n" if line else "#\n" for line in copying)
    out = [_NOTICE + notice + _DOCSTRING]
